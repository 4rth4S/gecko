...
def pull_misp (ioc_type, days):
  ...
  if str(ioc_type).upper() == 'URL':
    data_pull = {"request": {"type": "url", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  elif str(ioc_type).upper() == 'MD5':
    data_pull = {"request": {"type": "md5", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  elif str(ioc_type).upper() == 'SHA256':
    data_pull = {"request": {"type": "sha256", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  elif str(ioc_type).upper() == 'DOMAIN':
    data_pull = {"request": {"type": "domain", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  elif str(ioc_type).upper() == 'IPDST':
    data_pull = {"request": {"type": "ip-dst", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  elif str(ioc_type).upper() == 'IPSRC':
    data_pull = {"request": {"type": "ip-src", "last": str(days)+"d", "enforceWarnlinglist": "True"}}
  else:
